...
class DSPChannel:
    ...
    Filter coefficients are pre-computed and cached; only recomputed on set_filters().
    ...
    def _init_dsp(self, sample_rate: int):
        ...
        self.delay_ptr = 0
        # Pre-computed reverb delay indices (constant for a given sample_rate)
        self._D_L = int(0.060 * sample_rate)
        self._D_R = int(0.085 * sample_rate)

        # Pre-compute and cache filter coefficients
        self._recompute_filter_coefficients()

    def _recompute_filter_coefficients(self):
        """Recomputes and caches IIR filter coefficient arrays. Called only on parameter change."""
        alpha_lp = float(min(1.0, max(1e-6, 2.0 * np.pi * self.lowpass_cutoff / self.sample_rate)))
        self._b_lp = np.array([alpha_lp])
        self._a_lp = np.array([1.0, -(1.0 - alpha_lp)])

        b1_hp = float(np.exp(-2.0 * np.pi * self.highpass_cutoff / self.sample_rate))
        self._a0_hp = (1.0 + b1_hp) / 2.0
        self._b1_hp = b1_hp
        self._b_hp = np.array([self._a0_hp, -self._a0_hp])
        self._a_hp = np.array([1.0, -b1_hp])
    ...
    def _apply_dsp(self, out: np.ndarray, frame_count: int) -> np.ndarray:
        ...
        Filter coefficients are cached and not recomputed here.
        """
        ...
        # --- 2. Lowpass filter (cached 1-pole exponential smoothing IIR) ---
        x_lp = np.empty_like(out)
        for ch in range(2):
            zi = np.array([self.lp_state[ch]])
            y, zf = lfilter(self._b_lp, self._a_lp, out[:, ch], zi=zi)
            x_lp[:, ch] = y
        ...
        # --- 3. Highpass filter (cached 1-pole IIR) ---
        # Transposed direct form II initial condition:
        # zi[0] = b[1]*x_prev - a[1]*y_prev = -a0*x_prev + b1*y_prev
        x_filtered = np.empty_like(x_lp)
        for ch in range(2):
            zi = np.array([-self._a0_hp * self.hp_x_state[ch] + self._b1_hp * self.hp_y_state[ch]])
            y, _ = lfilter(self._b_hp, self._a_hp, x_lp[:, ch], zi=zi)
            x_filtered[:, ch] = y
        ...
        if feedback > 0.0:
            ...
            read_idx_l = (self.delay_ptr - self._D_L + i_vec) % self.delay_buffer_size
            read_idx_r = (self.delay_ptr - self._D_R + i_vec) % self.delay_buffer_size
